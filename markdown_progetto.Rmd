...
\begin{tabular}{l@{\hskip 8em}r}
1. \textbf{Abstract} & \textbf{X} \\
2. Obtaining data & X \\
3. Data pre-processing & X \\
4. Exploratory analyses & X \\
5. Descriptive analyses & X \\
\hspace{1em} 5.1 Global analyses & X \\
\hspace{1em} 5.2 Analyses by source & X \\
\hspace{1em} 5.3 Analyses by macroregion & X \\
\hspace{1em} 5.4 Green Score with focus on the sources & X \\
\hspace{1em} 5.5 Green Score with focus on the macroregions & X \\
6. Modeling & X \\
Bibliography and links & X \\
\end{tabular}
...
4. **Feature addition**, computed over *main* in paragraph 5.3. The new categorical variable groups the world countries into six macroregions.
...
### 5.3 Analyses by macroregion

We are now interested in analizying the generation of electricity by LC sources in different areas. To do so, we aggregate world countries in six macroregions on the basis of geographical, economical and cultural factors.
...
2. **Latin America and Caribbean**: North and South America's countries, except of USA and Canada.
...
2. **Hydropower** is an essential source of electricity in all the considered macroregion.

3. **Nuclear electricity** is significant only in developed countries and Eastern Europe; in those macroregions, its importance is comparable to hydropower generation.
...
### 5.4 Green Score with focus on the sources
...
\]
Note that in paragraphs 5.4 and 5.5 analyses we removed the countries with a smaller population than 500,000. The reason is that we expect those countries to be too small to have a significant independent electricity policy with respect to their neighbors.
...
### 5.5 Green Score with focus on the macroregions
...
```
The developed countries with the highest Green Scores are **Sweden**, Norway, Switzerland, and France, while the ones with the lowest values are Macao, Hong Kong, Israel and Malta.
...
It is also interesting to highlight that the developed Asian countries have a lower score compared to the others: the second best performing country, Japan, outperforms only Australia, Luxembourg, Cyprus and Malta.
...
ggplot(filter(place, tag == "east_europe"),
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
No country in Eastern Europe is near a Green Score equal to 1: the best-performing one is **Slovenia**, with a score of 0.72. Follow Slovakia, Albania and Armenia. In the region there are also three countries with a score near zero: Belarus, Azerbaijan and Moldova. As the barplot shows, Eastern European countries mainly produce LC electricity through hydro and nuclear sources.
...
2. **Kenya** is the only Sub-Saharan country with a good Green Score that generates significantly from non-hydropower sources, specifically geothermal [16]
...
```
We conclude the descriptive analyses by looking at the performance of Northern Africa and Middle East countries. Consistently with the findings at paragraph 5.3, the barplot shows that all the countries fail to reach a good performance: the best performing one, **Turkey**, has a Green Score smaller than 0.5. As the following table shows, this is mainly due to an large availability of oil and gas.
