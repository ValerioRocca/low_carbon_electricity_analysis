...
\hspace{1em} 3.1 Global exploratory analyses & X \\
\hspace{1em} 3.2 Variables transformation & X \\
\hspace{1em} 3.3 Plotting function and division in groups & X \\
\hspace{1em} 3.4 Analysis on groups of variables & X \\
...
\hspace{1em} 5.1 Initial data preparation for Modeling & X \\
\hspace{1em} 5.2 Functions definition & X \\
\hspace{1em} 5.3 Obtaining Models & X \\
...
The plots for reserves do not offer much insight; here follows the main findings: most countries do not have reserves; after transformation, the distribution between countries that have reserves is normal; different types of reserves are correlated.
...
Finally, we plot correlation among all variables, considering year as one of those. The main findings follow:

1. **Year** has a slightly positive correlation with **solar** and **wind**, as mentioned earlier, and a slight negative correlation with **coal**.
...
4. **Nuclear** is slightly positively correlated to **uranium reserves**, but it is equally correlated to economic indices (**GDP**, **HDI**).
...
We define the first two of them: one applies min-max normalization; the other contains a modified version of the logit function, which avoids infinite values since our data lies in the interval [0,1]

, and we define what columns we will later apply the functions to (in this case all numerical columns except **year**).
...
In the following step, we define the function *selectvar_nl*, which allows us to select which columns to include in the model as independent variables. The arguments are all boolean except the last one:

- *Bi*, identifies whether we want to include *Energy dataset*'s variables which regard specific energy sources;

- *Bt*, identifies whether we want to include **tag**;

- *By*, identifies whether the functions defined above to normalize and apply the logit function over the column grouped by year, or on the whole column;
...
The best model has R square of ~0.79, which is low with respect to our expectations, considering it has the variables for electricity from each source; it returns trivial coefficients, with a high impact of hydro (making up most of the renewable electricity) and also fossil fuel sources on the opposite end. GDP also has a high coefficient, and HDI is not in the model.
